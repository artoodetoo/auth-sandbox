...
use R2\Auth\ThirdpartAPI\OAuth\OAuthExceptionPHP;
...
class OAuthServer
{
    ...
    {
        if (!$timestamp) {
            throw new OAuthExceptionPHP('Missing timestamp parameter. The parameter is required');
        }

        // verify that timestamp is recentish
        $now = time();
        if (abs($now - $timestamp) > $this->timestamp_threshold) {
            throw new OAuthExceptionPHP("Expired timestamp, yours $timestamp, ours $now");
        }
    }
    ...
    {
        if (!$nonce) {
            throw new OAuthExceptionPHP('Missing nonce parameter. The parameter is required');
        }
