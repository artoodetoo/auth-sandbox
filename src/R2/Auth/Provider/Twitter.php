...
class Twitter extends OAuth1
{
    ...
    public function getUserProfile()
    {
        ...
        $this->user->profile->photoURL = (property_exists($response, 'profile_image_url'))
        ...
        foreach ($contactsids as $chunk) {
            ...
            if ($response && count($response)) {
                foreach ($response as $item) {
                    ...
                    $uc->identifier = (property_exists($item, 'id'))
                        ? $item->id
                        : "";
                    $uc->displayName = (property_exists($item, 'name'))
                        ? $item->name
                        : "";
                    $uc->profileURL = (property_exists($item, 'screen_name'))
                        ? ("http://twitter.com/" . $item->screen_name)
                        : "";
                    $uc->photoURL = (property_exists($item, 'profile_image_url'))
                        ? $item->profile_image_url
                        : "";
                    $uc->description = (property_exists($item, 'description'))
                        ? $item->description
                        : "";
    ...
    public function getUserActivity($stream)
    {
        ...
        foreach ($response as $item) {
            ...
            $ua->id = (property_exists($item, 'id'))
                ? $item->id
                : "";
            $ua->date = (property_exists($item, 'created_at'))
                ? strtotime($item->created_at)
                : "";
            $ua->text = (property_exists($item, 'text'))
                ? $item->text
                : "";

            $ua->user->identifier = (property_exists($item->user, 'id'))
                ? $item->user->id
                : "";
            $ua->user->displayName = (property_exists($item->user, 'name'))
                ? $item->user->name
                : "";
            $ua->user->profileURL = (property_exists($item->user, 'screen_name'))
                ? ("http://twitter.com/" . $item->user->screen_name)
                : "";
            $ua->user->photoURL = (property_exists($item->user, 'profile_image_url'))
